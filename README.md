...
5. 검색 속도 자체는 너비 우선 탐색(BFS)에 비해 느림.
</pre>

#### 깊이 우선 탐색(DFS)의 특징
* 자기 자신을 호출하는 순환 알고리즘의 형태를 지님. <br/>
* 전위 순회(Pre-Order Traversals)를 포함한 다른 형태의 트리 순회는 모두 DFS의 한 종류.
* 이 알고리즘을 구현할 때 주의할 점은 <br/>
  어떤 노드를 방문했었는지를 반드시 검사해야 하는 것이며, <br/>
  이를 검사하지 않을 경우 무한 루프에 빠질 위험이 있음.

#### 깊이 우선 탐색(DFS)의 과정
<img src='https://user-images.githubusercontent.com/59400030/88911031-39837580-d298-11ea-9e22-3ce814460ac4.png' width='50%'/><br/>
...
2. A와 인접한 노드들을 차례로 순회.
   A와 인접한 노드가 없다면 종료.
3. A와 이웃한 노드 B를 방문했다면, A와 인접한 또 다른 노드를 방문하기 전에
   B의 이웃 노드들을 전부 방문해야 함.
...
* 즉, 그래프 내에 적은 숫자의 간선만을 가지는 희소 그래프(Sparse Graph)의 경우 <br/>
